...
function IdCardletidate(idCard) {
    ...
    if (idCard.length == 15) {
        ...
        if(isletidityBrithBy18IdCard(idCard)&&isTrueletidateCodeBy18IdCard(a_idCard)){   //进行18位身份证的基本验证和第18位的验证
            return true;
        }else {
            return false;
        }
 ...
 * @return
 */
function isTrueletidateCodeBy18IdCard(a_idCard) {
    let sum = 0;                             // 声明加权求和变量
    ...
    if (a_idCard[17] == letideCode[CodePosition]) {
        return true;
    } else {
        return false;
    }
